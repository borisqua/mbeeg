...
// EEG raw data preprocessing math (filtering, epoching, reshaping, tools, etc...)
 ...
 **/
function epochs(stimuli, series, sampleRate, duration) {
 ...
 */
function butterworth4Bulanov(timeseries, samplingrate = 0, cutoff = 0, passthrough = false) {
 ...
 */
function detrend(timeseries, passtrough = false) {
  if (passtrough) {
 ...
 */
function reshape(series, start = 0) {
  let result = [];
 ...
 * @return {Array} - stream, buffer or object that contains vectors {timestamp, ch0, ch1, ... , chN} with modified values of N EEG channels
 */
function seriesCARExtension(series, start = 0, car = null) {
