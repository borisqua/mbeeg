...
function createWindows() {
  ...
  winConsole = window({
    width: 900,
    height: 550 + config.carousel.keyboard.viewport.rows * 32,
    // parent: winMain,
...
ipcMain
  ...
  .on(`ipcKeyboard-change`, (e, arg) => {
    ...
    epochSeries.reset(config.mbeeg.stimulation.sequence.stimuli);
    fs.writeFile(`config.json`, JSON.stringify(config, null, 2), err => { if (err) throw err; });
    config.carousel.ipc.command = "configUpdate";
    winKeyboard.webContents.send(`ipcKeyboard-command`, config);
    winConsole.webContents.send(`ipcKeyboard-command`, config);
    winMain.webContents.send(`ipcKeyboard-command`, config);
  })
  .on(`ipcConsole-command`, (e, arg) => {
    config = Tools.copyObject(arg);
    winMain.webContents.send(`ipcConsole-command`, config);
    winKeyboard.webContents.send(`ipcConsole-command`, config);
  });
