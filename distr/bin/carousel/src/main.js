...
ipcMain
  ...
  .on(`ipcKeyboard-stimulus`, (e, stimulus) => {
    if (!keyboardHidden) {
      ...
      // stimuli.pipe(ntFlashes).pipe(process.stdout);//.on('data', data => console.log(data));
      // console.log(`Flash:${stimulus[1]};${stimulus[0]} delta ${stimulus[0] - delta}`);//debug
      // delta = stimulus[0];//debug
      
      //or
      // serverForNT.write(stimulus);
    }
  })
